**
* RRF 2024 - Construction Data_Xuerong Lei
*-------------------------------------------------------------------------------	
...
*------------------------------------------------------------------------------- 
	...
	* Consumption in usd
	...
	foreach cons_var in food_cons nonfood_cons { //loop
...
*------------------------------------------------------------------------------- 	
	...
	collapse 	(sum) treat_cost /// 
				(max) read sick /// max(read) and max(sick)
				(mean) m_cost = treat_cost days_sick, by(hhid)
				// preserve, save "", restore
...
*------------------------------------------------------------------------------- 	
	...
	save "${data}/Final/TZA_amenity_analysis",replace
			//Save data
